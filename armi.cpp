...
Dictionary SimilarWord(vector<Dictionary> &word, string userWord) {
    /*int counter[v.size()];
    vector<string> sim;
    ...
    for (int i = 0; i < userWord.length; i++) {
        for (int j = 0; j < word.size(); j++) {
            for (int k = 0; k < word[j].wordEng.length; k++) {
                if (userWord.at(i) == word[j].wordEng.at(k)) {
                    counter++;
                }
            }

            if (counter >= userWord.length / 2) { tmp.wordEng += "\n" + word[j].wordEng; }
            counter = 0;

            for (int k = 0; k < word[j].wordHun.length; k++) {
                if (userWord.at(i) == word[j].wordHun.at(k)) {
                    counter++;
                }
            }

            if (counter >= userWord.length / 2) { tmp.wordHun += "\n" + word[j].wordHun; }
            counter = 0;

            for (int k = 0; k < word[j].wordSpa.length; k++) {
                if (userWord.at(i) == word[j].wordSpa.at(k)) {
                    counter++;
                }
            }

            if (counter >= userWord.length / 2) { tmp.wordSpa += "\n" + word[j].wordSpa; }
            counter = 0;
        }
    }
...
void fileRead(vector<Dictionary>& v) {
    ...
    while (moreSearch == 'y') {
        ...
        if (wordFound != 1) {
            ...
            Dictionary tmp = SimilarWord(v, userWord);
            
            cout << "Hasonlo szavak angolul: " << tmp.wordEng << endl;
            cout << "Hasonlo szavak magyarul: " << tmp.wordHun << endl;
            cout << "Hasonlo szavak spanyolul: " << tmp.wordSpa << endl;
        }
